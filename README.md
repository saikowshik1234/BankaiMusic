...
An offline music player for iOS and Android that plays the audio files already on your device. Built with Expo and React Native, BankaiMusic features a stunning interface with skeuomorphic elements: a turntable you scrub by hand, a coverflow library, and a Music Finder for identifying songs playing around you.
...
- Music Finder lets you identify what's playing nearby, using microphone capture. It saves your recent discoveries and gives you quick links to Spotify and Apple Music. This is the one feature that needs a network connection.
...
- expo-router for navigation (three tabs, with Settings, Onboarding, and Connect as slide-up modals)
...
- react-native-svg and expo-linear-gradient for the turntable
...
Scan the QR code with Expo Go, or press `a` / `i` to open an Android or iOS simulator. The device-wide media scan and Music Finder recording need a development or production build rather than Expo Go; importing files and the app-folder scan work in Expo Go.
...
Streaming Connect and Music Finder need credentials. Copy the example file and fill in your own:
...
app/                     expo-router routes
  ...
  (tabs)/finder.tsx      Music Finder (Song ID and history)
